...
solve do
  ...
  answer do |input|
    cubes = BitArray.new(101 * 101 * 101)
    input.each_line do |line|
      x1, x2, y1, y2, z1, z2 = line.match(/x=(-?\d+)..(-?\d+),y=(-?\d+)..(-?\d+),z=(-?\d+)..(-?\d+)\z/).not_nil!.captures.map(&.not_nil!.to_i)
      next unless x1 >= -50 && x2 <= 50 && y1 >= -50 && y2 <= 50 && z1 >= -50 && z2 <= 50
      case line
      when /\Aon/ ; (z1..z2).each { |z| (y1..y2).each { |y| cubes.fill(true, (z + 50) * 10201 + (y + 50) * 101 + x1 + 50, x2 - x1 + 1) } }
      when /\Aoff/; (z1..z2).each { |z| (y1..y2).each { |y| cubes.fill(false, (z + 50) * 10201 + (y + 50) * 101 + x1 + 50, x2 - x1 + 1) } }
      end
    end
    cubes.count(true)
  end
...
record Cuboid, x1 : Int32, x2 : Int32, y1 : Int32, y2 : Int32, z1 : Int32, z2 : Int32 do
  def intersects?(other : Cuboid)
    x2 > other.x1 && other.x2 > x1 &&
      y2 > other.y1 && other.y2 > y1 &&
      z2 > other.z1 && other.z2 > z1
  end

  def volume
    (x2 - x1).to_i64 * (y2 - y1) * (z2 - z1)
  end
  ...
  def difference(other : Cuboid)
    unless intersects?(other)
      yield self
      return
    end

    a = self & other
    xs = [self.x1, self.x2, a.x1, a.x2].sort!.uniq!
    ys = [self.y1, self.y2, a.y1, a.y2].sort!.uniq!
    zs = [self.z1, self.z2, a.z1, a.z2].sort!.uniq!
    xs.each_cons_pair do |x1, x2|
      ys.each_cons_pair do |y1, y2|
        zs.each_cons_pair do |z1, z2|
          part = Cuboid.new(x1, x2, y1, y2, z1, z2)
          yield part unless part == a
        end
      end
    end
  end
...
solve do
  ...
  answer do |input|
    ...
    input.each_line do |line|
      ...
      new_ons = [] of Cuboid
      ons.each do |on|
        on.difference(cuboid) do |part|
          new_ons << part
        end
      end
      new_ons << cuboid if line.matches?(/on/)
      ons = new_ons
    end
    ons.sum(&.volume)
  end
